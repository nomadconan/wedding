...
const ADMIN_NAV: NavItem[] = [
  ...
  // S8-09 이 이 링크를 살렸다 — 오래 죽어 있던 자리다(FIX-23).
  // **커뮤니티 신고와 합치지 않았다**(D-142) — 대상도 조치도 달라 한 목록에 섞으면
  // 처리 절차가 서로 다른 건이 같은 줄에 놓인다. 대신 서로의 열린 건수를 가리킨다.
  ...
  // C-1. **분쟁 조율 바로 위**에 둔다 — 조율은 "이 거래가 어디까지 왔나" 를 읽는
  // 데서 시작하고, 그 화면이 없어서 운영자는 **대시보드 집계 숫자**로만 거래를 봤다
  // (B-1 조사 3). 분쟁이 들어오기 **전에** 보는 화면이라 큐들보다 앞에 온다.
  { href: "/admin/transactions", label: "거래 조회", icon: Receipt },
  // S8-03 이 이 링크를 살렸다 — 오래 죽어 있던 자리다(FIX-23).
  { href: "/admin/disputes", label: "분쟁 조율", icon: Users },
  // S8-03. 위약금은 **분쟁 조율의 한 갈래**이고 큐가 이리로 넘긴다. 바로 아래에
