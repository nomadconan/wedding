...
> 상세 근거: `docs/07_개발명세서.md`(구현 기준), `docs/04_의사결정로그.md`(결정 이력 — **이 리포에는 없다**, §7.6),
...
| 면 | 기능 ID | 라우트 그룹 | 기능 수 |
|---|---|---|---|
| 소비자 | F-C-01 ~ F-C-37 | `(consumer)` / `(marketing)` / `(auth)` | 37 |
| 업체 어드민 | F-V-01 ~ F-V-19 | `(vendor)` | 19 |
| 운영자 콘솔 | F-A-01 ~ F-A-19 | `(admin)` | 19 |

> 이 표는 **명세서 §2 의 요약**이며 기능이 늘면 함께 고친다. 24/14/14 는 v1.0 시절 값이었고
> D-19(신규 13종) · D-27(쿠폰 4종) · D-26(커뮤니티 5종) · IDEA-02(준비 순서 뷰 1종)가 더해져
> 지금은 **37 / 19 / 19**(총 75)다.
> 값이 궁금하면 세지 말고 `docs/07_개발명세서.md` §2.1·§2.2·§2.3 의 절 제목을 본다 — 그쪽이 진실이다.
...
- **릴리즈 R1~R3 는 폐기됐다**(D-20). 명세서 §1.3 은 이제 **개발 순서 8단계**이며
  **단계별 공개가 없다** — 8단계 전부를 완성한 뒤 일괄 오픈한다. 단계 번호는 착수 순서일 뿐이고
  선행 조건만 충족되면 병행 착수한다. "R1 먼저" 같은 말이 나오면 낡은 정보다.
...
> 위 트리는 **05 문서 원문 그대로**이므로 실제 리포와 다음이 다르다. 알고 있을 것.
> · `docs/` 의 실제 구성은 **`03` · `05` · `06` · `07` + `TASKS.md` · `DESIGN.md` · `ASSETS.md` ·
>   `INVENTORY_CSV.md`** 다. `00`~`02`·`04` 는 이 리포에 없다(§7.6 — 특히 04 의사결정로그).
>   `06_개발환경_컨텍스트.md` 는 **루트가 아니라 `docs/` 안**에 있다.
> · `lib/core/` 는 트리에 적힌 셋(`rules`·`pricing`·`schemas`)보다 늘었다 — 도메인이 생길 때마다
>   폴더가 붙는다(장바구니·채팅·문의·상담·결제·업체 설정 등). **트리를 기준으로 "없는 폴더"라고
>   판단하지 말고 실제 디렉터리를 본다.**
...
| `npm run seed:accounts` | 로컬 시드 계정·픽스처 생성 (`db:reset` 뒤에 실행) |
| `npm run db:rls` | **RLS 격리 점검** — psql 세션을 갈아 끼워 DB 가 실제로 막는지 확인한다 |
| `scripts\dev-setup.bat` | 최초 셋업 일괄 실행 |

| `npm run verify` | 로컬 게이트 일괄 — lint · typecheck · test · 시크릿 · 커버리지 요약 · **문서 표 칸 수** · 자산 |
| `npm run audit:routes` / `audit:screens` / `audit:api` | 실동작 점검 — 정적 대조 / 실제 Chrome / 세션 쿠키로 라우트 전수 |
| `npm run check:escrow` / `check:settlement` / `check:no-store` | 배치 둘과 캐시 정책을 **실제 서버로** 확인한다 |

> 06 문서 §4 에는 앞의 8개까지만 적혀 있다. `seed:accounts`·`db:rls` 는 그 뒤에 생겼고,
> **DB 를 건드리는 작업의 검증 절차는 `db:reset` → `db:types` → `seed:accounts` → `db:rls`** 다.
>
> **`audit` 계열과 `check:escrow`·`check:settlement` 는 DB 에 쓰기를 남긴다.**
> 돌린 뒤에는 **반드시 `db:reset` → `seed:accounts`** 로 되돌린다 — 건너뛰고 `db:rls` 를
> 돌리면 깨진다(실제로 겪었다).
>
> **검증 명령에 파이프를 걸지 않는다.** 파이프가 앞 명령의 종료 코드를 가린다.
> bash 에서 걸어야 하면 `set -o pipefail` 을 **먼저** 켠다.
...
#### 표를 만지기 전 — 권한 **세 층**을 본다

정책만 보고 넘어가 뚫린 적이 여섯 번 있다. **정책은 맞았는데 그 아래가 열려 있었다.**

| 층 | 무엇을 보는가 | 실제로 뚫렸던 모양 |
|---|---|---|
| **1** | CHECK 부재 · **표 단위 GRANT** · cascade · **컬럼 권한** | `revoke insert (칸)` 만으로는 무효다 — **표에서 걷고 칸을 나열해 다시 준다.** 당사자가 배지를 스스로 켰고, 아무나 증적 표를 비웠다 |
| **2** | `exists (select 1 from <부모>)` 에 **소유자 조건이 없는 정책** | 부모가 열려 있으면 자식도 열린다 — 정책이 다른 표의 정책에 기대고 있었다 |
| **3** | **자격의 근거 표를 자격을 얻으려는 사람이 직접 쓸 수 있는가** | 플래너가 자기 위임을 만들 수 있었고, 아무나 `status='active'` 로 **처음부터 공개 플래너**가 될 수 있었다(`before update` 트리거는 INSERT 를 못 본다) |

**뚫린 곳이 없으면 그 사실도 `db:rls` 에 고정한다.** "봤는데 괜찮았다" 는 다음 사람에게
남지 않는다 — 검사로 남겨야 남는다.
...
- 지시문이 적은 **FIX 번호·내용·라우트 이름**이 `docs/TASKS.md` 와 다르면 **원장을 따르고
  그 사실을 보고한다.** 세 회차 연속으로 지시문의 근거가 낡아 있었고, 한 번은 지시문이
  적은 이름(`settlement-run`)을 그대로 썼다면 **첫 실행에서 CHECK 에 걸려 기록이 사라질**
  뻔했다(레지스트리·DB 어휘는 `settlement-aggregate` 였다).
- **원장도 낡는다.** 착수 전에 **그 항목이 지금도 사실인지 먼저 확인**한다 — 재현되지
  않으면 고칠 것이 아니라 원장을 고칠 것이다.

### 7.0b 검사가 헛돌지 않는지 스스로 점검한다

- **"없을 때 말하는가" 와 "있을 때 조용한가" 를 둘 다 본다.** 앞만 보면 **늘 경보하는
  코드도 통과한다.**
- **고친 것을 일부러 되돌려 놓고 검사가 FAIL 하는지 확인한다.** 되돌려도 통과하는 검사는
  아무것도 지키지 않는다.
- **수는 표 전체가 아니라 그 트랜잭션이 만든 상태로 센다.** `count(*)` 는 시드가 채워질 때
  조용히 뜻이 바뀐다 — 보안 검사 15개가 **빈 표 덕분에** 통과하고 있었다.
- **빈 목록·0 으로 통과하는 검사를 만들지 않는다.** 목록을 세는 검사에는 **"목록을 실제로
  읽었는가" 를 먼저 검사한다.**
...
- 새로운 의사결정이 나오면 **추가 항목(D-번호)을 제안**한다.
  - **`docs/04_의사결정로그.md` 는 이 리포에 없다.** 그러므로 초안은 (가) 최종 보고에 적고
    (나) `docs/07_개발명세서.md` **개정 이력**과 (다) `docs/TASKS.md` 의 해당 노트에 남기며
    (라) **`docs/DECISIONS.md` 에 항목을 추가한다**(T-00i 신설).
    로그 파일이 들어오면 그대로 옮긴다. **결정이 어디에도 안 적히는 상태로 두지 않는다** —
    D-26·D-27 이 실제로 그렇게 한동안 떠 있었고, 그 때문에 명세 없이 스키마를 만들 뻔했다.
  - **번호를 부여하는 자리는 `docs/DECISIONS.md` 하나다.** 태스크 노트와 개정 이력이
    각각 번호를 붙이다가 **D-29 가 서로 다른 두 결정에 겹쳤다**(Realtime · 장바구니).
    새 번호는 그 파일의 마지막 항목 다음 번호를 쓰고, 나머지 세 곳은 **그 번호를 인용만** 한다.
- **미결정 사항(O-01~O-15)을 임의로 확정하지 않는다.** 특히:
  - **O-02 수수료 요율** → `app_settings` 파라미터로만 접근. 코드에 하드코딩 금지.
  - **O-03 에스크로·계약서 법무** → 컬럼 정의만 유지, 집행 로직·조항 문안은 결론 대기.
  - **O-14 커뮤니티 운영 정책** → 모더레이션 기준·SLA·명예훼손 책임 범위. 구조만 만든다.
  - **O-15 수수료 산정 기준**(할인 전·후) → `app_settings.settlement.fee_basis`.
    **값이 없으면 정산 계산이 명시적으로 실패한다.** 기본값을 코드가 고르지 않는다.
  - 오픈 이슈의 현재 상태는 명세서 §부록의 **오픈 이슈 상태** 표가 갖는다.
