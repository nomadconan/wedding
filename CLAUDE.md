...
- 새로운 의사결정이 나오면 **추가 항목(D-번호)을 제안**한다.
  - **`docs/04_의사결정로그.md` 는 이 리포에 없다.** 그러므로 초안은 (가) 최종 보고에 적고
    (나) `docs/07_개발명세서.md` **개정 이력**과 (다) `docs/TASKS.md` 의 해당 노트에 남긴다.
