...
> ### 4단계 착수 순서 (S3-10b 정리)
>
> **선행이 이미 충족된 태스크** — S4-02(잔여) · S4-05(S4-01 완료로 풀림) · S4-06(S4-02 부분 완료분으로 충족) · S4-12(S2-03·S3-01 완료).
...
| sla-escalation (Cron, 1시간) | 배치 | S4-13 | 미착수 — **선행이 풀렸다.** 자리는 S4-01(`chat_rooms.awaiting_vendor_since` + `idx_chat_rooms_awaiting_vendor`), 눈금은 S4-04(`app_settings.chat.sla_response_minutes`), 행은 S4-04 부터 쌓인다. 남은 것은 배치 자체이며 `slaState()` 를 그대로 쓰면 된다 |
