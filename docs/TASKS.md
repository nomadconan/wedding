...
### 목적
PR 기반 개발 흐름을 고정하고, 개인정보·보안 사고를 자동으로 차단하는 게이트를 건다.
특히 **서비스롤 키·ANTHROPIC_API_KEY의 클라이언트 번들 유입**을 사람이 아니라 CI가 잡는다.

### 선행조건
- T-01 완료 (로컬에서 lint·test·build가 통과해야 CI에 걸 수 있다)
- GitHub 리포지토리 생성

### 작업 내용
- `git init` → 초기 커밋 → GitHub 원격 연결.
- `main`(배포) / `dev`(통합) 브랜치 생성. 기능 브랜치는 `dev`에서 분기.
- 브랜치 보호: `main`·`dev` 직접 push 금지, PR 필수.
...
### 완료 판정 기준
- [ ] `main` / `dev` 브랜치가 존재하고, 직접 push가 차단된다.
...
> `§3.8`(운영·시스템) 중 `feature_flags`·`app_settings`는 범위 축소 금지 원칙과 요율 파라미터화의
> 전제이므로 **포함 여부를 착수 전에 확인받는다**(하단 미결 항목 참조).
...
1. **`feature_flags` / `app_settings` / `audit_logs` 의 T-03 포함 여부**
   T-03 범위는 §3.1·§3.2·§3.5로 지정되어 있으나, `feature_flags`(범위 축소 금지 원칙의
   구현 수단), `app_settings`(요율·임계값 파라미터화), `audit_logs`(감사 필수)는
   §3.8에 있다. T-06의 초과 경고 임계값도 `app_settings` 를 전제로 한다.
   → **T-03에 §3.8 최소 3종을 포함할지 확인 필요.**

2. **온보딩 예산 초안의 `price_index` 의존**
