...
**신규 테이블 21개**(S4-01 이 `chat_room_reads` 를 더해 16 → 17, S5-01 이 `payment_webhook_events` 로 18, T-00e 가 쿠폰 3표로 21). 각 마이그레이션에는 RLS 정책을 같은 파일에 포함한다(§3.9, CLAUDE.md §5.5). `entity_events`는 **insert-only** — UPDATE·DELETE 정책을 부여하지 않는다. `chat_messages` 도 같은 이유로 **UPDATE·DELETE 권한을 회수**했다(수정·삭제 대신 회수 묘비).
