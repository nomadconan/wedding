...
**S3-01·S3-02**(온보딩 + 커플 연동)와 **S3-04**(장바구니·찜 스키마·RLS)가 끝났다.

다음은 **S3-03**(업체 탐색·필터)이다. S3-05(장바구니 화면)는 S3-03과 S3-04를 모두 선행으로 요구하는데 S3-04는 이제 풀렸으므로, **S3-03이 유일하게 남은 길목**이다. S3-03 → S3-05 → S3-06·S3-07 순으로 이어진다.
