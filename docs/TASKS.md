...
>
> **O-11(실시간 채팅 인프라 — Supabase Realtime vs 별도 웹소켓)이 필요한 지점은 S4-04 하나다.** 그것도 **전송 계층에 한정**된다.
> · **결정 없이 되는 것**: S4-01(스키마·RLS) · 메시지 저장·조회·읽음 처리 API · 채팅 화면(새로고침·폴링 기준) · S4-05 문의게시판(실시간이 아니다) · S4-12 · S4-06.
> · **결정이 있어야 되는 것**: 상대 메시지의 **즉시 반영**, 그리고 S3-05 가 `CartView`·`WishlistView` 에 남긴 `TODO(O-11)` 의 커플 실시간 공유. 두 자리가 같은 결정을 기다리므로 **한 번 정하면 둘이 함께 풀린다.**
> · S3-05 에서 쓴 방식(내 변경은 즉시, 상대 변경은 새로 고칠 때 + 그 사실을 화면에 적기)을 채팅에도 그대로 쓰면 O-11 전에도 사용 가능한 화면이 나온다.
...
>
> **정렬 3종이 아직 닫혀 있다.** `EXPLORE_SORT_PENDING`(`lib/core/schemas/explore.ts`)에 코드·이유·담당 태스크가 적혀 있고 화면에도 그대로 노출된다. 후기(S8-02)·응답 속도(S4-12)·예약 가능일(S2-05 보급) 이 채워지면 그 배열에서 빼고 `EXPLORE_SORTS` 에 넣는다. **참가격 대비 편차 순은 S3-08 에서 열었다** — 지수가 없는 지역의 업체는 빼지 않고 맨 뒤에 두며 '비교 기준 없음' 으로 적는다.
