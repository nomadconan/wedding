...
**S3-01·S3-02가 끝났다**(온보딩 6문항 + 커플 연동). 다음은 **S3-04**(장바구니·찜 마이그레이션)와 **S3-03**(업체 탐색·필터)이다. S3-03은 S2-03이 이미 끝나 선행이 모두 풀렸고, S3-04는 T-03만 있으면 되므로 두 태스크는 서로를 기다리지 않는다. 다만 S3-05(장바구니)가 둘 다를 선행으로 요구하므로 **S3-04를 먼저 두는 편이 흐름이 짧다.**
