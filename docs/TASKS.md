...
| T-04b | 디자인 시스템 기반 | [x] | §6 공통 UI 규칙, §7.5 | **S1-01로 승계.** ⚠ `feat/T-04b-design-system` 브랜치에 있고 **`dev` 병합 전**이다. 05·06·07이 `docs/DESIGN.md` 를 참조하므로 **이 PR을 먼저 병합**해야 한다 |
...
| S1-02 | 가격 표시 컴포넌트 v2 | [ ] | S1-01 | §6 — 플래너 수수료 포함 총액 표시 |
| S1-03 | 중개자 지위 고지 컴포넌트 | [ ] | S1-01 | §6, D-24 |
...
**S1-02 · S1-03** — 1단계 기반의 잔여분이다. S1-01(디자인 시스템)은 완료됐으나 v2.0에서 확정된 두 가지가 아직 컴포넌트에 반영되지 않았다.

1. **S1-02 가격 표시 컴포넌트 v2** — 현재 `PriceDisplay`는 총액·부가세·추가금까지만 다룬다. D-17의 **플래너 부분 선택 수수료**가 총액에 포함될 수 있으므로 표시 구조를 확장해야 한다(§6 "추가금·플래너 수수료 포함 여부를 같은 블록에서 확인").
2. **S1-03 중개자 지위 고지 컴포넌트** — D-24. 거래 관련 화면에 "통신판매중개자이며 계약 당사자가 아님"을 고지하는 공통 컴포넌트가 필요하다. `AiDisclaimer`와 같은 방식(접기 불가·고정 노출)으로 만든다.

그다음은 **S5-01 → S5-02**(요율 구조)다. **S2-03 상품·판매가 등록이 요율 조회에 의존**하기 때문이다 — 판매가 등록 화면에 예상 정산액을 보여주려면 요율 해석 엔진이 먼저 있어야 한다. 2단계를 시작하기 전에 5단계 앞부분을 당겨오는 것이 유일한 역순 의존이다.
