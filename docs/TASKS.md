...
| T-04b | 디자인 시스템 기반 | [x] | §6 공통 UI 규칙, §7.5 | **S1-01로 승계** — 아래 참조 |
...
| S8-05 | **CI 파이프라인** (구 T-02b) | [ ] | S1-01 | §7.2, §7.5 |
...
| ESLint `no-restricted-imports` | `lib/core`의 React/Next import 금지 규칙. 현재는 테스트가 대신 강제 | S8-05 |
| 커버리지 80% 측정 | `@vitest/coverage-v8` 미설치 | S8-05 |
