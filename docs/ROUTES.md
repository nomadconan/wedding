...
> | 축 | 값 | 무엇이 답하나 |
> |---|---|---|
> | **있다** | 화면 **101** · `route.ts` **133** · DB 표 **111** | 정적 대조 — `npm run audit:routes`. **명세 §6·§4 와의 차집합이 0** 이다(T-00n · D-205). 명세에만 남은 하나는 **아직 안 만든 상품 상세**(C-2c)라 맞는 상태다 |
> | **열린다** | 화면 **1,010 건**(101 × 계정 10) · **하드 실패 0** | `audit:screens` — 실제 Chrome 이 로그인 폼을 지난다. **C-2b 는 화면을 늘리지 않았다**(기존 상세 안에 붙였다) |
> | **열린다(API)** | **1,340 건**(라우트 134 × 계정 10) · **오류 0** | `audit:api` |
> | **맞다** | — | **자동으로 못 센다.** 열린 화면이 옛 가격을 말하고(FIX-22) 조항을 지우고 있었다(FIX-31) — 둘 다 200 이었다 |
> | **이어진다** | **`chain:walk` 28/28 · `vendor:walk` 29/29** (2026-09-20 · C-2b · 로컬·CI 양쪽) | 한 거래를 끝까지 · 업체가 반복하는 일. **둘 다 CI 가 돈다**(D-201). ~~이 Windows PC 에서는 각각 5/28 · 0/1~~ **FIX-77 이 닫았고 C-2b 회차에 이 PC 에서 다시 쟀다 — 28/28 · 29/29.** 원인은 제품이 아니라 메모리였다(D-208) |
>
