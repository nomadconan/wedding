...
>
> **측정일** 2026-09-03(최초) · **마지막 실주행** 2026-09-18(FIX-67 회차) · **분모 재측정** 2026-09-18(T-00n) ·  **측정 대상** `npm run build` 로 만든 프로덕션 서버
> (`http://localhost:3000`) · **측정 방법** 실제 Chrome 을 CDP 로 몰아 로그인 폼을 지나
> 라우트를 하나씩 열었다.
>
> **지금 값 (2026-09-18 · T-00n 확인)**
>
> | 축 | 값 | 무엇이 답하나 |
> |---|---|---|
> | **있다** | 화면 **102** · `route.ts` **135** · DB 표 **111** | 정적 대조 — `npm run audit:routes`. **명세 §6·§4 와의 차집합이 0** 이다. **C-2c 가 마지막 하나(상품 상세)를 채웠다** — 이제 '명세에만 있는 경로' 가 없다 |
> | **열린다** | 화면 **1,020 건**(102 × 계정 10) · **하드 실패 0** | `audit:screens` — 실제 Chrome 이 로그인 폼을 지난다. **가짜 픽스처 0건** — C-2c 가 `productId` 픽스처를 더했다(없을 때는 상품 상세를 **가짜 id 로 열고 200 으로 세고 있었다**) |
> | **열린다(API)** | **1,370 건**(라우트 137 × 계정 10) · **오류 0** | `audit:api` |
> | **맞다** | — | **자동으로 못 센다.** 열린 화면이 옛 가격을 말하고(FIX-22) 조항을 지우고 있었다(FIX-31) — 둘 다 200 이었다 |
> | **이어진다** | **`chain:walk` 60/60 · `vendor:walk` 39/39** (2026-09-21 · C-4c · 로컬·CI 양쪽) | 한 거래를 끝까지 · 업체가 반복하는 일. **둘 다 CI 가 돈다**(D-201). ~~이 Windows PC 에서는 각각 5/28 · 0/1~~ **FIX-77 이 닫았고 C-2b 회차에 이 PC 에서 다시 쟀다 — 28/28 · 29/29.** 원인은 제품이 아니라 메모리였다(D-208) |
>
> **화면 판정 분포는 아래 §8 표가 갖는다.** 이 머리말은 분모와 축만 적는다 —
> 두 벌로 적으면 한쪽만 고쳐지는 날이 온다(실제로 그랬다).
>
> **축이 다섯인 이유.** 처음에는 '열린다' 하나였다. 그런데 **화면 97개가 전부 200 이고**
> **API 1,320건이 전부 맞는 상태에서 한 거래가 UI 로 끝까지 가지 못했다**(FIX-65) — 계약 발행
> API 를 부르는 자리가 리포에 없었고, **부를 자리가 없는 API 는 라우트 점검도 화면 점검도**
> **통과한다.** 그래서 '이어진다' 가 생겼다.
...
> ### 로컬 실주행이 되살아났다 (FIX-77 **해소** · 2026-09-19 · D-208)
>
> 직전까지 이 Windows PC 에서 `chain:walk` 이 **5/28**, `vendor:walk` 이 **0/1** 이었다.
> **원인은 제품도 스크립트 로직도 아니고 이 PC 의 메모리 고갈**이었다 —
> 물리 **7.8GB** · 여유 **0.4GB** · 커밋 **26GB**(페이지파일로 계속 스왑).
>
> 그 상태에서 **덜 그려진 화면을 CDP 가 읽어** `업체가 후보에 없음` 이 났고,
> 그 문구는 **후보 계산이 틀렸다**로 읽힌다 — 회차 하나를 제품 추적에 썼다.
>
> **고친 것 넷** — ① Chrome 을 **트리째** 끝낸다(Windows 에서 `proc.kill()` 은 부모만
> 죽여 자식 열두어 개가 고아로 남았다) + 임시 프로필 삭제 + 주행 전 고아 청소
> ② `npm run stack:slim`(주행이 안 쓰는 컨테이너 셋 · 약 450MB) ③ 로그인 대기
> **40초 → 120초**(`WALK_LOGIN_TIMEOUT_MS`) ④ **고정 대기를 걷었다** —
> `vendor:walk` 에 남아 있던 `sleep(1200)` 을 *행이 생길 때까지 묻는* 방식으로 바꿨다.
>
> **그리고 실패가 무엇을 봤는지 말하게 했다**(D-208) — 후보 상자가 **0개면 화면이
> 안 그려진 쪽**, **다른 업체만 있으면 계산이 틀린 쪽**이다. 같은 문구가 두 가지를
> 뜻하면 이 문서가 거짓말을 한다.
| **C-1** (거래 사슬) | **+3** — `/contracts/[id]` · `/vendor/bookings/[id]` · `/admin/transactions` | 131 → **132** · 1,320 | `POST /api/bookings` — **리포에서 `bookings` 에 INSERT 하는 유일한 경로** |
| **C-3** (업체 편의) | 한 칸도 안 바뀜 | 132 → **133** · 1,330 | `POST /api/vendor/products/[id]/duplicate` |
| **FIX-66** (문의 생성 화면) | **+1** — `/inquiries/new` | 133 · 1,330 | **사슬의 첫 칸.** 그 화면이 없어 실주행 둘이 **세션 fetch 로 우회**하고 있었다(D-200) |
| **T-00n** (문서 현행화) | 한 칸도 안 바뀜 | 133 · 1,330 | **문서만 다뤘다** — 분모를 다시 세고 명세 차집합을 메웠다. **실주행은 다시 돌리지 않았다**(Docker 스택을 안 올렸다) |
| **C-2a** (카테고리 두 축) | 한 칸도 안 바뀜 | 133 · 1,330 | **칸만 더했다** — 표·API·화면을 만들지 않았다. `db:rls` **+21 → 1,731** |
| **C-2b** (상품 본문·사진) | 한 칸도 안 바뀜 | 133 → **134** · **1,340** | `POST/PATCH/DELETE /api/vendor/products/[id]/media`(한 라우트 · 메서드 셋). 화면은 **기존 `/vendor/products/[id]` 안**에 붙었다(사진 관리 · 완성도 권유). `db:rls` **+35 → 1,766** · `vendor:walk` **22 → 29** |
| **C-2c** (상품 상세) | **+1** — `/explore/[vendorId]/[productId]` | 134 → **135** · **1,350** | `GET /api/products/[id]`(공개). **명세에만 있던 마지막 경로가 채워졌다.** `db:rls` **+32 → 1,798** · `vendor:walk` **29 → 30**(FIX-78). **가짜 픽스처 10건을 없앴다** — `productId` 가 픽스처 목록에 없어 상품 상세가 **가짜 id 로 열리고 있었다** |
| **C-2d** (컨셉 태그) | 한 칸도 안 바뀜 | 135 · 1,350 | **칸만 더했다**(`products.style_tags`) — 표·API·화면을 만들지 않았다. 탐색 필터가 **업체 단위에서 상품 단위로** 바뀌었고 상속 규칙이 붙었다(D-213). `db:rls` **+30 → 1,828** · `vendor:walk` **30 → 35** |
| **C-2e** (상품 후기) | 한 칸도 안 바뀜 | 135 · 1,350 | **칸 1 + 트리거 1**(`reviews.product_id`) — 표·API·화면을 만들지 않았고 **상품 상세 안**에 후기 카드가 붙었다. `db:rls` **+32 → 1,860** · `chain:walk` **28 → 33**(후기 작성 → 상품 상세) |
| **C-4c** (태스크 ↔ 정보 연결) | 한 칸도 안 바뀜 | 136 → **137** · **1,370** | `GET /api/tasks/links`(신설 · F-C-39). **`tasks` 에 칸을 더하지 않았다** — 연결은 전부 유도된다(D-233). 화면은 **기존 `/checklist` 카드 안**에 다리 셋이 붙었고 `/community` 에 `?prep=` 필터와 글쓰기 칸 하나가 늘었다. `db:rls` **+25 → 2,045** · `chain:walk` **55 → 60**(펴기 · 셋을 눌러 닿기 · 살 것 없음 문장 · 준비 단계 필터 · API 대조) |
| **C-4e** (알림 → 화면 이동) | 한 칸도 안 바뀜 | 136 · 1,360 | **표도 칸도 안 건드렸다** — `db:types` 재생성에 **변화 0줄**. 레지스트리가 **템플릿 31종 전수**가 됐고 발송부 셋에 `bookingId` 가 붙었다(해지·안전거래·결제 — 화면이 예약 아래다). 화면은 **기존 `/notifications` 안**이며 **읽는 사람의 역할**을 링크 판정에 넘긴다(업체 멤버도 이 화면에서 읽는다). `db:rls` **+14 → 2,020** · `chain:walk` **53 → 55**(기한 알림 말고도 링크 · 업체 면으로 갈림) |
| **C-4d** (기한 알림) | 한 칸도 안 바뀜 | 135 → **136** · **1,360** | `POST/GET /api/jobs/task-due-notifications`(신설 · `vercel.json` 등록). 화면은 **기존 `/notifications` 안**에 이동 링크가 붙었다. `db:rls` **+44 → 2,006** · `chain:walk` **45 → 53**(파라미터 차단 · 발송 · 재실행 중복 · 완료 제외 · 네 갈래 · 증적 · 링크 눌러 열기) |
| **C-4b** (상품 리드타임) | 한 칸도 안 바뀜 | 135 · 1,350 | **칸 2 + CHECK 2 + 파라미터 키 1.** 화면은 **기존 `/explore/[vendorId]/[productId]` 안**에 붙었고(주문 기한 카드) 업체 폼에 두 칸이 늘었다. `db:rls` **+31 → 1,962** · `vendor:walk` **35 → 39**(업체가 적고 소비자가 역산된 날짜를 본다) |
| **C-4a** (준비 항목·예식 후) | 한 칸도 안 바뀜 | 135 · 1,350 | **칸도 안 더했다** — 어휘(6→9)와 **값**(템플릿 19→25)만 늘었고 CHECK 셋이 붙었다(FIX-82). 타임라인에 '예식 후' 구간 셋이 생겼지만 **화면은 기존 `/checklist` 안**이다. `db:rls` **+27 → 1,931** · `chain:walk` **38 → 45**(생성 → 예식 후 구간 → 라벨 → 표현 넷 → 빠진 항목 안내) |
| **C-2f** (지역 코드) | 한 칸도 안 바뀜 | 135 · 1,350 | **칸을 더하지도 않았다** — `region_code` 는 이미 있었고 **값의 어휘**가 생겼다(73종 · CHECK 5 · 권한 회수 1). `/prices/[region]` 이 **없는 지역에 404** 를 내기 시작했고(그전에는 200 + 빈 상태) `/explore?region=` 이 **어휘 밖 값에 422** 다. `db:rls` **+44 → 1,904** · `chain:walk` **33 → 38**(지역 필터 · 시도 접두어 · 옛 링크 · 오탈자 · 404) |
>
...
| 목록 | 수 (착수 시점) | 수 (**지금** · T-00n) | 무엇인가 |
|---|---|---|---|
| 실제 `app/**/page.tsx` | **97** | **101** | **이 점검의 분모** |
| 실제 `app/**/route.ts` | **128** | **133** | **API 점검의 분모.** `/api` 132 + `/auth/callback` 1 |
| 명세 `docs/07` §6 의 경로 행 | 83 | **102** | 문서가 약속한 화면. **T-00n 이 18개를 세워 차집합을 메웠다**(D-205). 102 인 것은 **아직 안 만든 상품 상세** 하나 때문이다 |
| 커버리지 검증표 D(`docs/TASKS.md`) | 87 | **102** | 태스크 대응을 적은 세 번째 목록 |

**분모는 `page.tsx` 101 개 · `route.ts` 133 개다**(착수 시점에는 97·128 이었다). 근거 셋 —
...
2. ~~**문서 둘은 서로도 다르다.** 명세 83 · 커버리지 87 이고 차집합이 양방향으로 있다.~~
   → **T-00n 이 닫았다.** 그때는 둘 중 하나를 분모로 삼으면 **실제로 배포되는 화면 중 세지 않는 것**이 생겼다 —
   `/support`·`/admin/ops`·`/bookings` 처럼 실재하고 라우팅되는 화면들이었다. **지금은 셋 다 §6 에 있다.**
3. **직전 회차가 지적한 수(87 vs 97 vs 127)는 세 목록이 아니라 네 목록이었다.**
   87 은 명세가 아니라 **커버리지 표**의 수이고 명세는 83 이다. 127 은 그 뒤 한 건이
   늘어 지금 **133** 이다. 이 문서의 수는 전부 `npm run audit:routes` 가 센 값이다.
