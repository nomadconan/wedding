...
> | **이어진다** | ⚠ **이 PC 에서는 `chain:walk` 5/28** · `vendor:walk` 는 아래 참조 | 한 거래를 끝까지 · 업체가 반복하는 일. **둘 다 CI 가 돈다**(D-201) — **판정 권한은 CI 에 있다**(아래) |
>
...
> ### ⚠ `chain:walk` 이 이 Windows PC 에서 4단계부터 무너진다 (C-2a · 2026-09-19 · **FIX-77**)
>
> `/inquiries/new?vendor=<id>` 에서 `#vendor-<id>` 를 못 찾아 **"업체가 후보에 없음"** 으로 죽고
> 그 뒤 24단계가 줄줄이 무너진다(**5/28**).
>
> **C-2a 가 만든 것이 아니다 — 확인했다.** 작업분을 `git stash -u` 로 **전부 치우고**
> (마이그레이션 0075 도 빠져 `tasks.vendor_category` 칸이 없는 상태) 같은 DB·같은 절차로
> 다시 돌렸더니 **같은 단계에서 같은 문구로 5/28** 이었다. **`dev` 의 상태다.**
>
> **CI 는 같은 주행을 리눅스에서 돌고 직전 PR(#119)에서 green 이었다**(integration 9m4s).
> **그래서 이 문서의 '이어진다' 축은 CI 를 권위로 삼는다** — 로컬 수치를 실동작 결과로
> 적지 않는다. 원인·후보는 `docs/TASKS.md` 의 **FIX-77** 에 있다.
