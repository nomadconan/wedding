...
> | **열린다** | 화면 **1,010 건**(101 × 계정 10) · **하드 실패 0** | `audit:screens` — 실제 Chrome 이 로그인 폼을 지난다 |
> | **열린다(API)** | **1,330 건**(라우트 133 × 계정 10) · **오류 0** | `audit:api` |
> | **맞다** | — | **자동으로 못 센다.** 열린 화면이 옛 가격을 말하고(FIX-22) 조항을 지우고 있었다(FIX-31) — 둘 다 200 이었다 |
> | **이어진다** | **`chain:walk` 28/28 · `vendor:walk` 22/22** (CI · 2026-09-19 · PR #120) | 한 거래를 끝까지 · 업체가 반복하는 일. **둘 다 CI 가 돈다**(D-201). ⚠ **이 Windows PC 에서는 각각 5/28 · 0/1 이다** — 아래 참조 |
>
...
> ### ⚠ 실주행은 **CI 에서만 판정된다** (C-2a · 2026-09-19 · **FIX-77**)
>
> | 어디 | `chain:walk` | `vendor:walk` |
> |---|---|---|
> | **CI (ubuntu)** | **28/28** | **22/22** |
> | 이 Windows PC | **5/28** | **0/1** |
>
> **로컬에서 무너지는 자리** — `chain:walk` 은 `/inquiries/new?vendor=<id>` 에서
> `#vendor-<id>` 를 못 찾아 **"업체가 후보에 없음"** 으로 죽고 그 뒤 24단계가 줄줄이
> 무너진다. `vendor:walk` 은 **로그인 단계에서 CDP 가 죽는다.**
>
> **C-2a 가 만든 것이 아니다 — 두 방향으로 확인했다.**
> ① 작업분을 `git stash -u` 로 **전부 치우고**(마이그레이션 0075 도 빠진 상태) 같은
>    DB·같은 절차로 돌렸더니 **같은 단계에서 같은 문구로 5/28** 이었다 — **`dev` 의 상태다.**
> ② **같은 커밋이 CI 에서 28/28 · 22/22 로 통과했다**(PR #120 · integration 9m6s).
>
> **그래서 이 문서의 '이어진다' 축은 CI 수치를 쓴다.** 로컬 수치를 실동작 결과로 적지
> 않는다 — 환경이 못 도는 것과 제품이 안 도는 것은 **다른 사실**이고, 섞으면 이 문서가
> 거짓말을 한다. 원인·후보는 `docs/TASKS.md` 의 **FIX-77** 에 있다.
>
> **다음 회차에 로컬 실주행이 필요하면 FIX-77 을 먼저 본다** — 안 그러면 같은 시간을 잃는다.
