...
> 계정 열 축 모두에게 401 이다.
>
> **`FIX-22`(서버 조회 캐시) 뒤 재측정** — 화면 970·API 1,310 **둘 다 한 칸도 안
> 바뀌었다**(라우트도 131 그대로). 캐시를 끈 것이 화면 판정을 바꾸지 않았다는 뜻이다.
> **다만 이 회차는 계정별로 쪼개 돌렸다** — 점검 도구가 화면 하나에서 굳는 일이
> 세 번 있었고(`FIX-59` 신설) 계정 단위로 나누면 굳은 축만 다시 돌릴 수 있다.
> 열 축을 모두 돌려 합친 수이며 빠뜨린 축은 없다.
>
> **`FIX-31`(이름 마스킹 정밀도) 뒤 재측정** — 화면 970·API 1,310 **또 한 칸도 안
> 바뀌었다.** 마스킹은 서버 안에서만 도는 순수 함수라 화면 판정에 나타나지 않는다 —
> 그 대신 저장되는 **인용**이 달라지며, 그것은 `lib/core/masking` 의 케이스 표와
> 업체 자가 진단 실동작으로 확인했다.
