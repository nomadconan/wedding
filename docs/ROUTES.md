...
> | **열린다(API)** | **1,350 건**(라우트 135 × 계정 10) · **오류 0** | `audit:api` |
> | **맞다** | — | **자동으로 못 센다.** 열린 화면이 옛 가격을 말하고(FIX-22) 조항을 지우고 있었다(FIX-31) — 둘 다 200 이었다 |
> | **이어진다** | **`chain:walk` 53/53 · `vendor:walk` 39/39** (2026-09-21 · C-4d · 로컬·CI 양쪽) | 한 거래를 끝까지 · 업체가 반복하는 일. **둘 다 CI 가 돈다**(D-201). ~~이 Windows PC 에서는 각각 5/28 · 0/1~~ **FIX-77 이 닫았고 C-2b 회차에 이 PC 에서 다시 쟀다 — 28/28 · 29/29.** 원인은 제품이 아니라 메모리였다(D-208) |
>
