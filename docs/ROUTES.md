...
| 커버리지 검증표 D(`docs/TASKS.md`) | 87 → **98** | 태스크 대응을 적은 세 번째 목록. **점검 시점 87 이었고 실재 화면 11 개가 빠져 있었다** — S0-04 가 넣어 98(실화면 97 + 미착수 `/contracts/[id]`)로 맞췄다 |
...
2. **문서 둘은 서로도 달랐다.** 명세 83 · 커버리지 87 이고 차집합이 양방향으로 있었다.
...
**표를 손으로 옮겨 적지 않는다.** 아래 넷이 이 문서의 표를 만든다.
...
3. **`/design-system` 은 프로덕션에서 404 가 정상이다**(S8-05). 점검할 때만
   `ENABLE_DEV_ROUTES=true` 로 연다.
4. **API 단계는 쓰기를 친다.** `GET` 이 없는 핸들러는 빈 본문으로 `POST`/`PATCH` 를
...
| `404` | 없는 것을 가리켰다. **`※` 표시가 있으면 시드에 그 행이 없어 가짜 id 로 연 것**이다 | 표시에 따라 다르다 |
...
| 판정 | 건수 | 읽는 법 |
|---|---|---|
| 정상 | **307** | 내용과 함께 그려졌다(카탈로그 10 포함) |
| 빈 상태 | **310** | `EmptyState` — **데이터가 없어서**이며 화면은 제 일을 했다 |
| 권한 거부 | 126 | 화면 가드가 `?denied=1` 로 되돌렸다(의도) |
| 로그인 요구 | 79 | 미들웨어가 `/login?next=…` 으로 보냈다(의도) |
| 404 | 57 | **38 은 시드에 행이 없어 가짜 id 로 연 것**(§4.5) · **19 는 RLS 가 남의 행을 감춘 결과**로 정상이다 |
| 오류 상태(전제 코드) | 82 | 아래 |
| 리다이렉트 | 9 | `/admin/consultation-disputes` → `/admin/disputes?source=consultation`(설계) |
| **하드 실패** | **0** | — |

**RLS 가 만든 404 를 결함으로 세지 않는다.** `/bookings/[id]`·`/reports/[id]`·
`/vendor/products/[id]` 를 **실제 id** 로 열면 주인에게는 `정상`, 남에게는 `404` 가 떴다 —
행이 안 보이니 화면이 `notFound()` 를 부른 것이고 **그것이 옳은 동작**이다.
예: 예약 상세는 `couple-linked-a/b`(고객)와 `vendor`·`staff`(업체) 넷에게만 열렸다.
...
### 4.5 점검이 닿지 못한 곳 — 시드 픽스처 공백

**이 문서에서 가장 중요한 한 줄이다.** `seed:accounts` 가 만들지 않는 표가 열이라
화면 다섯을 **실제 데이터로 열어 본 적이 없다**:

| 화면 | 필요한 표 | 이번에 확인한 것 |
|---|---|---|
| `/chat/[roomId]` | `chat_rooms` | 없는 id → 404. **정상 경로 미확인** |
| `/community/[postId]` | `community_posts` | 없는 id → 404. **정상 경로 미확인** |
| `/share/[token]` | `share_links` | 없는 토큰 → 닫힌 안내. **정상 경로 미확인** |
| `/rsvp/[token]` | `guests` | 없는 토큰 → 404. **정상 경로 미확인** |
| `/vendor/invite/[token]` | `vendor_invites` | 없는 토큰 → 404. **정상 경로 미확인** |

`consultations`·`contract_cancellations`·`tasks`·`price_rules`·`product_options` 도
0행이라 그 API 들이 **'없음' 경로로만** 확인됐다.
**`FIX-56`(실서비스 전 필수)** 으로 등록했다 — 이대로 열면 한 번도 뜨는 것을 본 적 없는
화면 다섯을 배포하게 되고, 그것이 D-75·D-76 이 막으려던 상태 그 자체다.

### 4.6 API — 5xx 0 · 비로그인 경계는 전부 지켜졌다

`route.ts` **128** 개를 계정 열 축으로 쳤다(**1,280 건**). `GET` 이 있으면 `GET` 을,
...
정직하게 적는다. 표가 '전수' 라고 말하더라도 **다음 둘은 확인되지 않았다.**
...
2. **화면 다섯의 정상 경로**(§4.5 · `FIX-56`). 시드에 행이 없어 **없는 id 로만** 열었다.

이 둘을 적어 두지 않으면 다음 사람이 이 문서를 "전부 확인됐다" 로 읽는다 —
...
| 등급 | 번호 | 한 줄 | 담당 |
|---|---|---|---|
| **실서비스 전 필수** | **FIX-56** | 시드가 만들지 않는 픽스처 때문에 **화면 다섯이 실제 데이터로 열린 적이 없다** | **S0-02 확장** |
| 기록 | **FIX-55** | 명세 §6 과 실제 라우트가 **열여섯 자리**에서 어긋나 있다(§6 에만 1 · 실제에만 15) | 문서 태스크 |
| 기록 | **FIX-57** | `/vendor/bookings` 가 **없는 화면**(`/contracts/[id]`)으로 가는 링크를 그린다 | **S5-05** |
| 기록 | **FIX-58** | `npm run dev` 가 Windows 에서 **전 라우트를 500 으로 만든다**(개발 서버 문제) | 없음(도구) |
...
1. **`FIX-56` 을 먼저 한다.** 시드에 다섯 표의 행을 하나씩 넣으면 화면 다섯의 정상 경로가
   처음으로 열린다. 나머지 FIX 를 고칠 때도 그 화면을 **실제로 볼 수 있게** 된다.
2. **실서비스 전 필수 나머지 여덟** — `FIX-08`·`11`·`12`·`14`·`22`·`31`·`34`·`42`.
...
| `/chat` | 로그인 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 |
| `/chat/[roomId]` ※ | 로그인 | 404 | 404 | 404 | 404 | 404 | 404 | 404 | 404 | 404 |
| `/checklist` | 로그인 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 |
...
| `/community` | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 |
| `/community/[postId]` ※ | 404 | 404 | 404 | 404 | 404 | 404 | 404 | 404 | 404 | 404 |
| `/community/write` | 로그인 | 정상 | 정상 | 정상 | 정상 | 정상 | 정상 | 정상 | 정상 | 정상 |
| `/consultations` | 로그인 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 |
...
| `/rsvp/[token]` ※ | 404 | 404 | 404 | 404 | 404 | 404 | 404 | 404 | 404 | 404 |
| `/search` | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 |
| `/share/[token]` ※ | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 |
...
| `/vendor/cancellations` | 로그인 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 |
| `/vendor/chat` | 로그인 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 |
| `/vendor/community` | 로그인 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 |
| `/vendor/compliance` | 로그인 | 빈 | 빈 | 빈 | 빈 | 빈 | 정상 | 정상 | 빈 | 빈 |
| `/vendor/consultations` | 로그인 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 | 빈 |
...
| `/vendor/invite/[token]` ※ | 로그인 | 404 | 404 | 404 | 404 | 404 | 404 | 404 | 404 | 404 |
...
| `GET /api/community/posts/[id]` | 404 | 404 | 404 | 404 | 404 | 404 | 404 | 404 | 404 | 404 |
| `GET /api/community/posts/[id]/comments` | 404 | 404 | 404 | 404 | 404 | 404 | 404 | 404 | 404 | 404 |
...
| `POST /api/community/posts/[id]/like` | 401 | 403 | 403 | 403 | 403 | 403 | 403 | 403 | 403 | 403 |
| `POST /api/community/posts/[id]/scrap` | 401 | 403 | 403 | 403 | 403 | 403 | 403 | 403 | 403 | 403 |
