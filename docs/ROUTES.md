...
> 라우트를 하나씩 열었다.
>
> **지금 값 (2026-09-06 · T-00m 확인)**
>
> | 축 | 값 |
> |---|---|
> | 화면 | **970 건** (화면 97 × 계정 10) · **하드 실패 0** |
> | 화면 판정 | 정상 **377**(카탈로그 10 포함) · 빈 상태 **273** · 권한 거부 **126** · 오류 상태 **82** · 로그인 요구 **79** · 404 **24** · 리다이렉트 **9** |
> | API | **1,310 건** (라우트 131 × 계정 10) · **오류 0** |
> | API 판정 | 검증 거절 **340** · 403 **310** · 200 **299** · 401 **214** · 404 **137** · 307 **10** |
>
> **시드를 바꾸면 이 표는 낡는다.** 다시 돌리는 방법은 §2 에 있다.
>
> <details><summary><b>재측정 이력 — 회차별 증감</b></summary>
>
> | 회차 | 화면 | API 라우트 · 건수 | 비고 |
> |---|---|---|---|
> | `FIX-56` (시드 픽스처) | 판정이 바뀐 회차 — 정상 **+70** · 404 **−33** | 128 · 1,280 | 없던 데이터가 생겨 화면이 그릴 것이 생겼다 |
> | `FIX-12` (요율 무효화) | 한 칸도 안 바뀜 | 128 → **129** · 1,290 | `POST /api/admin/commission-rates/void` |
> | `FIX-11` (오픈 준비) | 한 칸도 안 바뀜 | 129 · 1,290 | `/admin/ops` 에 절이 하나 늘었을 뿐 |
> | `FIX-14` (에스크로 릴리즈) | 한 칸도 안 바뀜 | 129 → **130** · 1,300 | 배치 라우트 |
> | `FIX-08` (정산 집계) | 한 칸도 안 바뀜 | 130 → **131** · 1,310 | 배치 라우트 |
> | `FIX-22` (서버 조회 캐시) | 한 칸도 안 바뀜 | 131 · 1,310 | **계정별로 쪼개 돌렸다**(`FIX-59`) |
> | `FIX-31` (이름 마스킹) | 한 칸도 안 바뀜 | 131 · 1,310 | 마스킹은 서버 안 순수 함수라 화면 판정에 안 나타난다 |
>
> **배치 라우트 셋은 계정 열 축 모두에게 401 이다** — 세션이 아니라 `CRON_SECRET`·서비스롤
> 키로 인가되기 때문이며(D-149) 형제 여덟과 같은 값이다.
>
> **'열린다' 와 '맞다' 는 다른 축이다.** 970 을 통과한 화면이 **옛 가격을 말하고
> 있었고**(`FIX-22`) 갖춰진 조항을 **지우고 있었다**(`FIX-31`). 둘 다 200 이었다.
> 이 표는 **렌더가 성공했다**는 뜻이지 **값이 옳다**는 뜻이 아니다.
>
> </details>
...
| 목록 | 수 (착수 시점) | 수 (**지금** · T-00m) | 무엇인가 |
|---|---|---|---|
| 실제 `app/**/page.tsx` | **97** | **97** | **이 점검의 분모** |
| 실제 `app/**/route.ts` | **128** | **131** | **API 점검의 분모**(FIX-12·14·08 이 하나씩 더했다) |
| 명세 `docs/07` §6 의 경로 행 | 83 | 83 | 문서가 약속한 화면 |
| 커버리지 검증표 D(`docs/TASKS.md`) | 87 | **98** | 태스크 대응을 적은 세 번째 목록 |

**분모는 `page.tsx` 97 개 · `route.ts` 131 개다**(착수 시점에는 128 이었다). 근거 셋 —
...
| `npm run check:no-store` | **서버 조회가 굳지 않는지 실제로 본다**(FIX-22). 값을 바꾸고 화면이 따라오는지, 디스크 캐시(`.next/cache/fetch-cache`)에 금액·권한 조회가 쌓이는지 센다. **굳어야 하는 `/guides` 가 그대로 굳는지도 함께 본다** — 앞만 보면 캐시를 통째로 꺼 버린 코드도 통과한다 | 종료 코드 |
