...
export type EntityType =
  ...
  | "planner"
  // S6-04. 열람 권한 위임. **무엇을 열었는지 memo 에 적지 않는다**(§7.3) — 행이 이미
  // 갖고 있고 옮겨 적으면 두 곳이 갈린다. 남길 사실은 "제안했다·수락했다·거절했다·
  // 거뒀다" 라는 전이와 **범위 개수**이며, 그 전이가 곧 "언제부터 언제까지 남이 우리
  // 예산·하객을 봤는가" 의 답이 된다(D-23). 앱 경로가 여기서 남기고, 표를 직접
  // 두드린 쓰기는 0069 의 트리거가 대신 남긴다 — 둘이 겹치지 않게 갈라 두었다.
  | "planner_engagement"
  ...
  // S7-09. 하객·좌석. **이름을 memo 에 넣지 않는다**(§7.3) — 하객은 우리 사용자가
  // 아니고 명단은 커플이 옮겨 적은 제3자 정보다. 남길 사실은 인원 수·테이블 수뿐이며
  // **초대 토큰도 담지 않는다**(이벤트에 토큰이 남으면 링크가 로그로 새는 것과 같다).
  | "guest"
  | "seating_plan"
  | "profile"
  ...
  | "dispute"
  // S8-10. 참가격 지수 재계산. **사유 본문을 memo 에 넣지 않는다**(§7.3) — 남길 사실은
  // **표본 수**다. 지수를 움직인 것은 상태 변경이고, 표본이 몇이었는지가 나중에
  // "그때 무엇으로 냈나" 를 답한다(S7-04 가 위약금 시뮬레이션에서 정한 것과 같은 규칙).
  | "price_index"
  // S8-11. 검증 후기와 그 신고. **본문·답변·사유 문안을 memo 에 넣지 않는다**(§7.3) —
  // 행이 이미 갖고 있고 옮겨 적으면 두 곳이 갈린다. 남길 사실은 "썼다·거뒀다·
  ...
  | "ticket"
  // S8-12. 피처 플래그 전환. **사유 본문을 memo 에 넣지 않는다**(§7.3) — `audit_logs` 가
