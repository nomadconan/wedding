...
/**
 * 상담 일정 제안 카드 — **자리는 만들고 발송은 만들지 않는다.**
 *
 * F-C-27 은 "카드에서 바로 F-C-29 예약으로 연결" 이라고 쓴다. 그런데 F-C-29(상담·
 * 탐방 예약)는 **S4-07** 이고 `consultations` 테이블조차 아직 없다(S4-02 잔여).
 * 지금 카드를 발송할 수 있게 만들면 누를 수 있는 버튼이 **없는 화면**을 가리킨다 —
 * `BottomTabNav` 주석이 "없는 화면을 가리키는 탭은 만들어 두고 켜지 않는 것이 아니라
 * **깨진 것을 켜 둔** 상태다" 라고 정리한 그 문제다.
 *
 * 그래서 이렇게 갈랐다.
 ...
 */
export const CONSULTATION_CARD_PENDING = {
  kind: "consultation_proposal" as SystemCardKind,
  label: "상담 일정 제안",
  reason: "상담·탐방 예약을 아직 만들지 않아 이 카드에서 예약으로 넘어갈 수 없어요.",
  filledBy: "S4-07",
} as const;
