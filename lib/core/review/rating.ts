...
/**
 * **업체 평점은 상품 평점의 평균이 아니다.**
 *
 * 같은 함수(`rateVendor`)에 **그 상품의 후기만** 넘기면 상품 평점이고, **그 업체의
 * 후기 전부**를 넘기면 업체 평점이다. 둘은 같은 규칙·같은 가중이며 **분모만 다르다.**
 *
 * ── 왜 상품 평균의 평균이 아닌가 ────────────────────────────────────────────
 * 상품 A 에 후기 1건(5.0), 상품 B 에 후기 3건(3.0)인 업체를 생각한다.
 *  · 상품 평균의 평균 → (5.0 + 3.0) / 2 = **4.0**
 *  · 후기 전체의 평균 → (5 + 3 + 3 + 3) / 4 = **3.5**
 * 앞쪽은 **후기 한 건짜리 상품이 세 건짜리 상품과 같은 무게**를 갖는다. 그러면
 * 상품을 잘게 쪼갤수록 평점이 올라가고, 그건 업체가 한 일이 아니라 **상품 등록
 * 방식**이 순서를 정하는 것이다. 이 파일이 축을 접을 때 세운 규칙("한 사람이 한 표")
 * 을 한 층 위에 그대로 적용한다.
 *
 * ── 상품이 하나인 업체와 열인 업체 ──────────────────────────────────────────
 * **상품 수는 무게에 영향을 주지 않는다. 후기 수가 무게다.**
 *
 * ── 캐시 컬럼을 만들지 않는다 ───────────────────────────────────────────────
 * S8-11 이 `planners.rating_avg` 를 쓰지 않기로 한 것과 같은 이유다 — 저장하면 두
 * 곳이 생기고, **갈리는 날 어느 쪽이 맞는지 화면으로는 모른다.** 읽을 때 센다.
 */
export const RATING_COMPOSITION = {
  code: "reviews_flat_v1",
  label: "후기 단위 평균",
  rules: [
    "상품 평점은 그 상품의 후기만, 업체 평점은 그 업체의 후기 전부를 셉니다.",
    "업체 평점은 상품 평점을 다시 평균 내지 않습니다 — 그러면 후기가 적은 상품이 과대 대표됩니다.",
    "상품 수는 무게에 영향을 주지 않습니다. 후기 수가 무게입니다.",
    "평점을 저장해 두지 않고 볼 때마다 셉니다.",
  ],
} as const;
 ...
 * 상품 평점 옆에 반드시 붙는 문장.
 *
 * `ratingCaption` 과 나눈 이유는 **분모가 다르다는 것을 화면이 말해야** 하기
 * 때문이다 — "검증 후기 3건" 이 업체 전체인지 이 상품인지 구분되지 않으면
 * 같은 숫자가 두 가지를 뜻한다.
 */
export function productRatingCaption(rating: VendorRating): string {
  if (rating.reviewCount === 0) return "이 상품에는 아직 검증 후기가 없습니다.";

  return `이 상품 검증 후기 ${rating.reviewCount}건 기준 · ${RATING_BASIS.label}`;
}
