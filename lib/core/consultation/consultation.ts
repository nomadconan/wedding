...
/**
 * **화면이 생겼는데 문구가 안 따라왔다**(C-1b).
 *
 * 예전에는 "운영자 조율 화면은 준비 중이에요(S4-10)" 였다. 그런데 S8-03 이 네 출처를
 * `/admin/disputes` 한 화면으로 모았고 `/admin/consultation-disputes` 는 그리로
 * 보내 준다 — **다 만든 기능을 화면이 '준비 중' 이라 말하고 있었다.**
 *
 * 사용자에게는 "아무도 안 본다" 로 읽힌다. 실제로는 보고 있다.
 */
export const DISPUTE_QUEUE_NOTE =
  "양측 답변이 달라 운영자 조율 대기로 넘어갔어요. 운영자가 증적을 보고 판정합니다.";

/**
 * 3자 일정 공유(D-19 · F-C-29).
 *
 * **막는 이유가 바뀌었는데 문구가 그대로였다**(C-1b). 예전에는 "플래너 마켓을 아직
 * 만들지 않아" 였고 담당이 `S6-02` 였는데, **S6-02~S6-05 로 마켓·위임·범위가 전부
 * 섰다.** 남은 것은 상담에 플래너를 지정하는 자리와 외부 캘린더이며 그것은 `S4-11`
 * 이다 — 화면은 **끝난 태스크를 담당으로 가리키고 있었다.**
 *
 * `consultations.planner_id` 칸과 플래너 열람 정책은 S4-07 이 이미 만들어 두었다.
 */
export const PLANNER_SHARE_PENDING = {
  label: "플래너와 일정 공유",
  reason: "상담에 플래너를 지정하는 자리가 아직 없어요. 위임은 이미 할 수 있어요.",
  filledBy: "S4-11",
} as const;
