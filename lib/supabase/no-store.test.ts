 ...
 *
 * ── 왜 검사가 필요한가 ──────────────────────────────────────────────────────
 * **Next 14 에서는 살아 있는 구멍이었다.** 14 는 `fetch` 를 기본으로 캐시하고
 * supabase-js 는 그 `fetch` 로 PostgREST 를 부른다. 클라이언트를 하나 새로 만들 때
 * `no-store` 를 안 적으면 **그 조회는 조용히 1년짜리 캐시에 얹혔다**
 * (`revalidate: 31536000`). 로컬에서 확인했다 — `products.base_price_total` 을
 * 바꿨는데 `/explore` 가 옛 가격을 그렸다.
 *
 * 실수가 **눈에 안 보인다는 것이 그 결함의 핵심**이었다. 안 적으면 아무 경고도 없고
 * 화면은 정상으로 보이며, 값이 바뀌는 날에야 드러난다.
 *
 * ── Next 15 에서 전제가 바뀌었다 (FIX-61) ───────────────────────────────────
 * **15 부터 `fetch` 기본값이 `no-store` 다.** 그래서 오늘은 `no-store` 를 빠뜨려도
 * 굳지 않는다 — 이 검사의 역할이 **'구멍 막기' 에서 '의도 고정'** 으로 바뀌었다.
 *
 * **그래도 남긴다.** 이유가 셋이다:
 *  1. `next: { revalidate }` · `force-cache` 는 여전히 켤 수 있고, 켠 사람이 금액·권한
 *     조회에 켰는지는 아무도 안 본다.
 *  2. **프레임워크 기본값은 또 바뀐다.** 실제로 14 → 15 에서 바뀌었고, 그때 이 검사가
 *     없었다면 FIX-22 를 다시 겪는다.
 *  3. 이 파일은 규칙이 아니라 **목록**이다 — "이 리포에 서버 클라이언트를 만드는 자리가
 *     몇 개이고 각각 무엇을 정했나" 를 한눈에 둔다. 기본값과 무관하게 값을 한다.
 *
 * ── 왜 린트 규칙이 아니라 테스트인가 ────────────────────────────────────────
...
const CACHED_ON_PURPOSE: Record<string, string> = {
  "lib/content/loader.ts":
    "SEO 화면(/guides)은 정적으로 굳는 것이 목적이다(§2.1). 신선도는 페이지의 revalidate=300 이 잡고, 공개 데이터라 권한 문제가 아니다. Next 15 부터는 fetch 자체가 안 굳지만 굳는 주체는 원래 페이지(Full Route Cache)였고 그것은 그대로다 — 그래서 이 자리에 no-store 를 적지 않는 판단도 그대로다(FIX-61).",
};
