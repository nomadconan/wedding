 ...
 * **Next 14 에서는 이것이 살아 있는 구멍이었다.** 14 는 `fetch` 를 기본으로 캐시하고
 * supabase-js 는 그 `fetch` 로 PostgREST 를 부르므로, 서비스롤 조회가
 * **`revalidate: 31536000`(1년)** 으로 디스크 캐시에 얹혔다 — 로컬에서 확인했다:
 * `products.base_price_total` 을 바꿨는데 `/explore` 가 **옛 가격을 그대로 그렸다.**
 *
 * **Next 15 부터는 `fetch` 기본값이 `no-store` 다.** 그러니 이 줄이 없어도 오늘은
 * 안 굳는다. **그래도 지운다는 뜻은 아니다** — 역할이 '구멍 막기' 에서
 * **'의도 고정'** 으로 바뀌었을 뿐이다. `next: { revalidate }` 나 `force-cache` 는
 * 여전히 켤 수 있고, 프레임워크 기본값은 또 바뀐다(실제로 14 → 15 에서 바뀌었다).
 * **기본값에 기대는 대신 코드가 말하게 둔다.**
 *
