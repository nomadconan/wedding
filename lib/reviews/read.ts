 ...
 *
 * **이 함수가 없으면 `/reviews/new/[bookingId]` 는 도달 불가 화면이 된다.** §6.2 가
 * 진입점으로 삼을 만한 `/bookings/[id]`(예약 상세)는 아직 만들어지지 않았고(S5-06
 * 잔여), 화면을 만들고 가리키는 곳을 두지 않는 것이 이 리포에서 반복된 실수다
 * (FIX-25 가 세던 다섯). 그래서 `/me` 가 그 자리를 맡는다 — 예약 상세가 생기면
 * 거기로 옮기고 여기는 남겨 둔다(두 곳에서 들어와도 되는 종류의 화면이다).
 */
