...
export async function POST(request: NextRequest) {
  ...
  const { data: opened } = await admin
    .from("job_runs")
    .insert({ job_name: "price-index-refresh", started_at: startedAt, status: "running" })
    .select("id")
    .maybeSingle();
  ...
  try {
    ...
    const cells = [
      ...new Map(
        ((vendorRows ?? []) as { region_code: string; category: string }[]).map((row) => [
          `${row.region_code}|${row.category}`,
          row,
        ]),
      ).values(),
    ];
    ...
    if (jobRunId) {
      await admin
        ...
        .update({
          ...
          error_summary: insufficient > 0 ? `insufficient_sample:${insufficient}` : null,
        })
        .eq("id", jobRunId);
    }

    return ok({ cells: cells.length, built, insufficient, guestBucket: PRICE_INDEX_ALL });
  } catch {
    if (jobRunId) {
      await admin
        .from("job_runs")
        .update({ finished_at: new Date().toISOString(), status: "failed", error_summary: "refresh_failed:1" })
        .eq("id", jobRunId);
    }
